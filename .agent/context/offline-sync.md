...
- **Duplicado esperado en la tabla, ya no en la UI (2026-09-13)**: workout offline + claim+sync sigue pudiendo generar dos filas de PR para el mismo evento (PR local ya escrito + trigger SQL remoto al pushear el set) — sin dedup en la escritura ni en el push (`personal_records` no tiene `set_id` ni constraint único, y el push es un `insert` plano, no `upsert`). Se colapsa en la lectura: `getPersonalRecords`/`getAllPersonalRecords` usan una CTE con `ROW_NUMBER() OVER (PARTITION BY exercise_id, reps ORDER BY weight DESC, achieved_at ASC, id ASC)` para devolver una única fila canónica por `(exercise_id, reps)`. Las filas duplicadas siguen en SQLite (y en Supabase) — esto es un fix de lectura, no de escritura — y el equivalente remoto (`packages/database/src/repositories/progressRepository.ts`, usado solo por analíticas remote-only) no lleva el mismo dedup.
