...
- **Multi-dispositivo en modo invitado — duplicados de catálogo, arreglado (2026-09-25)**: mismo usuario invitado en dos dispositivos antes de vincular/crear cuenta → si cada uno había creado por su cuenta el mismo catálogo de ejercicios por defecto o las mismas medidas corporales por defecto (cada uno con sus propios UUIDs), `claimGuestIdentity` (solo reescribe `user_id`) dejaba ambas copias conviviendo como filas duplicadas. Nueva `mergeDuplicateCatalogEntries(db, userId)` (`packages/database/src/sync/mergeDuplicateCatalog.ts`) — llamada en `_layout.tsx` justo después de `claimGuestIdentity` + el `sync()` posterior (que trae lo que ya hubiera remoto de un claim anterior en otro dispositivo; es el único momento en que este dispositivo ve ambos orígenes a la vez). Colapsa por nombre normalizado (trim + lowercase) los duplicados vivos de `categories`, `exercises` (agrupados por `category_id` ya canónico + nombre) y `body_measurements`: se queda con el más antiguo (`created_at` asc, `id` asc de desempate), reescribe las FKs de las tablas que referencian al resto (`exercises.category_id`; `workout_exercises`/`routine_day_exercises`/`personal_records`/`exercise_goals`.`exercise_id`; `body_measurement_entries.measurement_id`) y tombstona las duplicadas — mismo patrón de cascada manual (transacción + `enqueuePendingOp` por fila) que `deleteCategory`/`deleteExercise` en `localExerciseRepository.ts`. Si hubo alguna fusión, se dispara un segundo `sync()` para subir tombstones/FKs antes de que la UI relea. Deliberadamente NO toca `workouts`/`routines`/`sets`: dos entrenamientos o rutinas con el mismo nombre en dispositivos distintos no son necesariamente el mismo dato — fusionarlos a ciegas podría borrar historial real; solo se fusiona lo que la propia app puede crear de forma determinista con el mismo nombre en dos sitios (catálogo por defecto, medidas por defecto) o donde un duplicado casi nunca es intencional (categorías/ejercicios por nombre exacto). Cubierto por `packages/database/src/sync/__tests__/mergeDuplicateCatalog.test.ts` (6 tests).

  **Verificado end-to-end en dispositivo físico (2026-09-25)**, con una cuenta real que ya tenía historial propio (no solo el escenario de dos guests): al iniciar sesión desde un dispositivo que era invitado, se disparó el claim y la fusión detectó y colapsó categorías exactamente duplicadas ("Abdominales", "Espalda") entre el catálogo local del dispositivo y el ya existente en la cuenta. Esto reveló dos bugs reales del `SyncEngine`, no relacionados con la fusión en sí — ver la entrada de `force-stop` de sesión en CLAUDE.md/`Bugs conocidos` para el detalle completo:
  - `executeOperation` usaba `.insert()` para `op_type === "insert"`, que no es idempotente: si el proceso se mataba entre el insert remoto (ya completado) y el `markOpSucceeded` local, el reintento fallaba para siempre con `duplicate key`. Arreglado a `.upsert()`.
  - `mergeDuplicateCatalogEntries` solo se llamaba dentro del flujo de claim (`handleSessionChange`), así que si el proceso se mataba durante el `runSync()` previo a esa llamada (fácil con un pull inicial grande), la fusión no se alcanzaba nunca y no había ningún otro punto que la reintentara. Arreglada moviéndola dentro de `runSync()` (`apps/mobile/app/_layout.tsx`) — se ejecuta en cada sync normal, no solo tras un claim, así que converge sola en el siguiente ciclo sin importar dónde se interrumpió el anterior.

  Nota sobre el propio dedup: `normalizeName` solo hace `trim()` + `toLowerCase()`, sin plegado de acentos — "Bíceps" y "Biceps" NO se consideran duplicados (se confirmó en la verificación: ambos sobrevivieron como categorías separadas). Es una limitación conocida y deliberada por ahora, no un bug — fusionar por similitud fonética/de acentos es más propenso a falsos positivos (colapsar categorías que el usuario quería mantener distintas) y no se pidió.
...
- **`createLocalProgressRepository`**: `getPersonalRecords`/`getAllPersonalRecords` (alimentadas por lo anterior) + `getWeeklyTraining`/`getBestSetsByExercise` (JOIN/GROUP BY simples sobre tablas ya locales, migradas también aunque fuera del plan original de la Fase 6 — sin ellas el resumen semanal de Progreso y el auto-check de goals sin PR de peso quedaban rotos en modo invitado) + `getChartData` (añadida 2026-09-11, ver más abajo).
- Pantallas: `(tabs)/progress.tsx`, `goals/index.tsx`, `calculators.tsx`, `workout/[exerciseId].tsx`, `exercise-history/[exerciseId].tsx` — todas al `progressRepo` local, sin split-repo.
- **Duplicado en la tabla arreglado en la escritura (2026-09-13 mitigado en lectura, 2026-09-24 arreglado de raíz)**: cualquier set completado offline-first y luego sincronizado generaba dos filas de PR para el mismo evento — no solo tras claim+sync, en **cualquier** sync de **cualquier** cuenta, porque el orden de push (`sets` antes que `personal_records`, `pushOrdering.ts`) garantizaba que el trigger SQL remoto se disparase siempre antes de ver la fila ya escrita localmente (`maybeRecordPersonalRecord`). **`010_drop_personal_record_trigger.sql`** quita ese trigger (`trg_update_personal_record`/`update_personal_record()`) — al ser offline-first, ninguna app aparte del móvil escribe `sets` en Supabase directamente, así que era puramente redundante; `maybeRecordPersonalRecord` + push ya cubren la generación de PRs sin él. **Aplicada a Supabase (2026-09-24)**, a mano por el usuario vía el SQL Editor del dashboard (`supabase db push` fallaba por conectividad IPv6 en la conexión directa; pegar el `DROP TRIGGER`/`DROP FUNCTION` en el editor lo evita). Las filas duplicadas generadas ANTES de esta migración siguen en SQLite y en Supabase (la migración no las limpia) — se siguen colapsando en la lectura con la CTE `ROW_NUMBER() OVER (PARTITION BY exercise_id, reps ORDER BY weight DESC, achieved_at ASC, id ASC)` de `getPersonalRecords`/`getAllPersonalRecords`, que se mantiene como red de seguridad permanente para ese histórico.

**Las dos fugas de lectura detectadas tras aplicar la migración, tapadas (2026-09-25)**: el equivalente remoto (`packages/database/src/repositories/progressRepository.ts`) no tenía el mismo dedup — código muerto hoy (`RepositoryContext` solo instancia el local), pero riesgo latente si se reutilizaba — y `exportBackup()` (`backupRepository.ts`) metía `personal_records` sin dedup en la copia de seguridad `.fitnotes` — fuga real y activa, cualquier cuenta con duplicados históricos exportaba un backup con PRs duplicados. Se añadió `dedupePersonalRecords()` (nueva función exportada de `progressRepository.ts`, mismo criterio de desempate que la CTE local: mayor peso, empate por `achieved_at` más antiguo y luego `id` menor) usada en tres sitios: `getPersonalRecords`/`getAllPersonalRecords` remotos (ahora async, devuelven `{data, error}` igual que el local — antes devolvían el query builder de Supabase sin ejecutar, sin ningún caller real que dependiera de encadenar `.order()` sobre él) y `exportBackup()`.

**Efecto secundario de la migración encontrado y arreglado en el mismo cambio**: `restoreBackup()` (`backupRepository.ts`) nunca insertaba `personal_records` explícitamente en su lista `insertSteps` — antes de quitar el trigger, no hacía falta: el propio trigger regeneraba la tabla como efecto secundario de insertar `sets`. Al quitarlo, una restauración de backup borraba `personal_records` (vía `DELETE_TABLES`) y nunca la volvía a poblar, perdiendo todos los PRs del usuario. Arreglado insertando `personal_records` desde el propio `BackupData` (después de `exercises`, su única FK), deduplicado también en este punto por si el `.fitnotes` que se restaura es de antes del fix de `exportBackup()`. Test nuevo `packages/database/src/repositories/__tests__/progressRepository.test.ts` (5 tests) cubre `dedupePersonalRecords` de forma aislada (sin duplicado, empate por peso, empate por `achieved_at`, empate por `id`, entrada vacía).
...
**2026-09-25**: tres bugs reales encontrados y arreglados en esas tres operaciones remote-only, todos divergencias con la lógica incremental normal de PRs que sí usan `deleteWorkout`/`removeExercise`/`deleteSet` (`prExercisesFromSetIds`/`prExercisesFromWorkoutExerciseIds` + `resyncPersonalRecordsForExercise`, `workoutRepository.ts`):
- `deleteWorkoutHistory` (borrado masivo desde Configuración, por rango de fechas y/o ejercicio) no llamaba a `resyncPersonalRecordsForExercise` tras borrar — dejaba PRs huérfanos de sets/entrenamientos ya borrados. Arreglado: recalcula cada ejercicio afectado del rango (o el único, si se filtró por `exerciseId`) tras el borrado.
- `recalculatePersonalRecords` (`backupRepository.ts`) reimplementaba la regla de PR a mano en vez de usar `recomputePersonalRecordLedger` (`packages/core/src/utils/personalRecords.ts`, la misma función que usa `resyncPersonalRecordsForExercise`), con dos divergencias: no fijaba `achieved_at` en el insert (caía al `DEFAULT now()` de la columna, fechando "hoy" todos los PRs recalculados) y excluía series de calentamiento (`is_warmup=false`), cuando la ruta incremental normal no lo hace. Arreglado reescribiéndolo sobre `recomputePersonalRecordLedger`.

Sin test para `deleteWorkoutHistory`/`recalculatePersonalRecords` — como el resto de los repos remotos, no tienen cobertura Vitest en este repo (necesitarían un fake de Supabase con selects anidados tipo `sets(weight, reps, created_at)`, que el fake actual, `sync/testing/fakeSupabaseClient.ts` —pensado solo para `SyncEngine`—, no soporta). Verificado por typecheck + revisión de código, apoyándose en que `recomputePersonalRecordLedger` sí tiene 14 tests propios en `packages/core`. No verificado en dispositivo físico por ser operaciones destructivas sobre la única cuenta real disponible en este entorno.
...
- `contexts/RepositoryContext.tsx` → `<RepositoryProvider>` construye los 8 repos locales una vez (`useMemo`) y resuelve `local_identity`: `useRepositories() → { db, workoutRepo, exerciseRepo, routineRepo, bodyTrackerRepo, goalsRepo, progressRepo, preferencesRepo, calendarRepo, userId, isGuest, refreshIdentity, wipeAndSetIdentity }`. Gatea el render hasta que `getLocalDb()` y la identidad resuelven; preferencias se hidratan en un efecto aparte (no gatean el render).
