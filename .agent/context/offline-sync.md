...
Plan completo en `/home/xabier/.claude/plans/precious-snuggling-shannon.md` (6 fases — la 7 original, bootstrap, se fusionó en la 5). Objetivo: CRUD completo sin red en mobile (workouts, ejercicios/categorías, rutinas, body tracker, goals), con **cuenta opcional** — la app funciona desde el primer arranque sin login, y crear/vincular una cuenta activa la sincronización. **Solo mobile** — web no cambia y sigue requiriendo cuenta.
...
| 6 | Personal records offline (réplica JS del trigger SQL) | ⬜ pendiente — más urgente ahora: un workout de invitado no dispara el trigger remoto hasta que hay claim+sync |
...
repositories/           localWorkoutRepository.ts, localExerciseRepository.ts, localRoutineRepository.ts,
                        localBodyTrackerRepository.ts, localGoalsRepository.ts, shared.ts
```
...
|---|---|---|
| `localWorkoutRepository` | workouts, workout_exercises, sets — CRUD completo | `exportAllCSV`, `shareWorkout`, `deleteWorkoutHistory` |
...
| `localGoalsRepository` | goals (`upsertGoal` traduce el `onConflict: user_id,exercise_id` remoto a `INSERT ... ON CONFLICT DO UPDATE` local) | — (`getAllPersonalRecords`/`getBestSetsByExercise` de `progressRepository` para el progreso, siguen remotos) |
...
- `apps/mobile/contexts/RepositoryContext.tsx` → `<RepositoryProvider>` construye los 5 repos locales una vez (`useMemo`) y resuelve `local_identity`, exponiendo `useRepositories()` → `{ db, workoutRepo, exerciseRepo, routineRepo, bodyTrackerRepo, goalsRepo, userId, isGuest, refreshIdentity, wipeAndSetIdentity }`. Gatea el render con loading state hasta que tanto `getLocalDb()` como la identidad resuelven.
...
- 69 tests en `packages/database` (14 archivos): local repos (workout/exercise/routine/body-tracker/goals), local identity, claim, migrations, serializeExecutor, sync (applyRemoteRows, pendingOpsQueue, pushOrdering, syncEngine), nodeSqlExecutor.
